...
export async function asyncIteratorNext(iterator) {
...
export async function* withLatestFrom(iterator, fromIterator) {
    ...
    for await (const items of select(iterator, fromIterator)) {
        for (const item of items) {
            if (item.index === 0) {
                if (item.done) return
                if (latestItem) yield [item.value, latestItem.value]
            }
            else if (!item.done) {
                latestItem = item
            }
...
export async function* merge(...iterators) {
    for await (const items of select(...iterators)) {
        for (const item of items) {
...
export async function* scan(fn, initialState, iterator) {
    let state = initialState
    yield state
    for await (const value of iterator) {
        state = await fn(state, value)
        yield state
    }
}
